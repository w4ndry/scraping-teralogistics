...
url = 'https://www.teralogistics.com/wp-admin/admin-ajax.php?action=uforwarders&p={}&ob=rate&s=&c=&pr=&mc=&ms=&sd='
...
def scrap_forwarder():
    ...
    total_urls = []
    total_page = get_total_page(url.format(1), __location__, 'forwarders.json')
    ...
    # save urls to json
    with open(os.path.join(__location__, 'forwarders_urls.json'), 'w+') as outfile:
        json.dump(total_urls, outfile)

    # open local urls
    with open(os.path.join(__location__, 'forwarders_urls.json')) as json_file:
    ...
    # get_details('https://www.teralogistics.com/forwarder/pt-dunia-express-transindo/#fwd', str(30))
